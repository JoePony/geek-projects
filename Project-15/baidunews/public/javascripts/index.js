$(function() {
    ...
    $('.delete').on('click',function(e){
            console.log('不确定',this.id);
        if(confirm('确定删除吗?')){
            var tokenV=$('#access_token').val();
            $.post(
                '/delete',
                {id:this.id, access_token:tokenV} //发送id和token校验值
            );
            window.location.href='admin'; //删除后刷新后台页面
        }
    ...
    function createListNews(d) {
        var title = filterXSS(d.title);                                 //对异步返回的相关新闻数据进行声明，转义防止xss
        var url = '/shownews/' + d.id;
        var img = filterXSS(d.imgPath);  //转义防止xss
        var source = filterXSS(d.source);
