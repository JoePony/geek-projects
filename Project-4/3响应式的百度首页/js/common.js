...
//移动端样式
function show_moreNav(){     //光标点击移动端导航的更多
    var nav_list=document.getElementById('nav_list');
    if(nav_list.className=='nav-list'){
        nav_list.className+=' nav-list-all';                        //显示更多
    }else{
        nav_list.className='nav-list';
    }
}
function refresh(){         //点击新闻换一批
    if(document.getElementById('refresh-icon').className=='i-rotate'){
        document.getElementById('refresh-icon').className='';
    }
    document.getElementById('refresh-icon').className='i-rotate';
}